...
func backgroundInputChecks(_ check: (Bool, String) -> Void) {
    ...
    check(contextSection.contains("[\"appName\": bound.appName, \"windowTitle\": title]") && contextKeys == ["documentName", "browserAddress", "visibleText", "visibleTextTruncated", "visibleTextNodes", "visibleTextMs", "visibleTextWalk", "selectedText", "windowCount", "openApps", "accessibility", "menus", "background"], "targetContext writes exactly the keys the runner's schema accepts (the three visibleText* keys are the page-text walk's stop and counts, src/core/context.ts): \(contextKeys.sorted())")
