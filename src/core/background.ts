 ...
 * the helper.
 */
import type { Action, ExecutionResult, Rung, TargetSpec } from "./schema";
 ...
 * rung the run or memory already saw this application ignore. Undefined for
 * a step that is not delivered to the window at all.
 */
export function backgroundLadder(
  action: Action,
  shortcutLabel: string | undefined,
  skips: (route: BackgroundRoute) => boolean,
): Ladder | undefined {
  const all = (rungs: Rung[], foreground: boolean): Ladder => ({
    rungs: rungs.filter((rung) => !skips(backgroundRoute(action, rung))),
    foreground,
  });
  switch (action.type) {
    case "click_control":
...
/** The user's hands are in the bound window (design §3). */
