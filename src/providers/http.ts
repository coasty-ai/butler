...
context.menus lists the frontmost application's own menus, one line per menu, with each item's shortcut in brackets: it is that application's complete list of what it can do, including commands that are nowhere on screen. Read it before improvising. Press an item with menu_item(path), for example {"type":"menu_item","frame_id":"<frame_id>","path":["Playback","Play"]}, or press the shortcut it shows. A menu item is resolved by name when it runs, so it never depends on pixels, and a greyed-out item tells you the application is not in the state you assumed (Spotify greys out Search while no window is open: open a window first, for example with Window > Spotify). Coordinates are fractions of the screenshot, never pixels: x = pixel_x / image_width_px and y = pixel_y / image_height_px. For example, a button at pixel (720, 450) in a 1440x900 image is x=0.5, y=0.5. Always copy frame_id exactly as given in the context.
