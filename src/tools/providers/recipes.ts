 ...
 *
 * A recipe that runs a Node package names it in `install` (package, pinned
 * version, bin) and runs `node` on the bin the app installed
 * (src/tools/install.ts): npx never runs a recipe, since inside the network
 * sandbox it asks the registry for the latest tag and hangs (measured
 * 2026-09-19). The pin is bumped here, by hand, with the package's bin name
 * checked (`@playwright/mcp` renamed its bin between 0.0.50 and 0.0.70).
 */
...
export const RECIPES: readonly ServerRecipe[] = [
  {
    ...
    privateLocal: true,
    consent:
      "Reads and, if you tick them, changes files inside {folder} only. No network: nothing leaves this Mac through it. Runs as you. Butler installs its package from npm when you test or approve it, and never again while it runs.",
    installNote:
      "Needs Node 22+. Butler installs @modelcontextprotocol/server-filesystem 2026.8.31 into its own folder when you test or approve; that step needs network.",
  },
  {
    id: "playwright",
    name: "Playwright",
    ...
    installNote:
      "Needs Node 18+ and the Playwright MCP Bridge extension in Chrome. Butler installs @playwright/mcp 0.0.82 into its own folder when you test or approve; that step needs network.",
  },
  {
    id: "github",
    name: "GitHub",
  ...
  },
  {
    ...
    installNote: "Needs the Claude Code CLI (claude) on this Mac.",
  },
];
 ...
 * (two folders) under two ids. A recipe with `install` gives the row
 * command "node" and its own arguments alone; the registry puts the
 * installed bin in front of them (src/tools/install.ts liveArgs).
 */
