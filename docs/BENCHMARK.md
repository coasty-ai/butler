...
- Otherwise the task's own grader runs. **If it cannot read what it needs, the result is `unknown`, never `passed`.** No accessibility text means `NO_ACCESSIBILITY`; no frontmost application means `NO_FRONTMOST_INFO`; a browser task with no address at all means `NO_BROWSER_ADDRESS`. The address is the surface's `domain` (the page's host), else `context.browserAddress` (the address field's value, only while that field has focus). In cycle `20260919-0816-a839d34` five of six Safari attempts were `NO_BROWSER_ADDRESS` while Chrome's had a host: Safari sets no window-level URL and holds its web area under an `AXTabGroup`, which the helper's bounded host walk (`webAreaHost`, `native/macos/Controller.swift`) skipped along with toolbars, so `domain` was never set there; the walk now enters tab groups and reads the web area's own `AXURL`. The same missing host had left the policy's protected-website hand-off silent in Safari; a page whose address still cannot be read is now reported (`hostUnknown`) and hands off while any site is protected (`HANDOFF_SURFACE`), so a Safari attempt that ends that way is the helper's read failing loudly, not a grade.
...
- **appr**: approvals the policy asked for. Every approval is an interruption a user would feel; driving this number down matters as much as the success rate. The result file says which questions, as codes (`approvalCodes`: `SAVE_CHANGES`, `PLACE_ORDER`, `CLICK_CONTROL`, ...), and how each was answered; a failed attempt's `reason` cell names the ones it was refused (`declined SAVE_CHANGES 2`). See docs/HARNESS_LOOP.md §6 for how to read them.
...
7. **The browser's fixture tabs** (`src/gym/bench/browser-reset.ts`). A sign-in fixture (`wall-login-mfa-handoff`) leaves its password field focused, and a focused secure field holds macOS secure event input for the whole session: the helper's `surface` then answers `secureInput: true` to every following run, the policy turns that into a takeover, and the attempt hands off with no action (cycle `20260919-0957`: 14 of 15 attempts, `NO_END_STATE`, until the operator loaded `about:blank` in the tab). So after every attempt with a browser the harness does what the operator did: it asks the browser chosen for the attempt, the benchmark's own by `chooseBrowser`'s rule (`benchOwnBrowser`: not running at the start, or with no window of yours), to load `about:blank` in every tab whose URL is on the fixture server's origin, `http://127.0.0.1:<port>`, and in no other tab: a tab on any other host, or on another loopback port (a dev server of yours), is never navigated, and no tab is ever closed. Safari, Safari Technology Preview, Chrome, Chrome Canary, Edge and Brave are scripted (one Apple Event each, by bundle id, the origin passed as the script's argument); a browser with no tab scripting is left as it is and the row says so. The row carries `browserReset: { tabs, code? }` (`NO_SCRIPT`, `NOT_RUNNING`, `UNREAD` for a browser that did not answer), never a URL. Nothing is reset after real input or a stop. Should secure input still be on when the next attempt comes up, the gate waits as `SECURE_INPUT` and, when the holder is the benchmark's browser, applies this same reset once before waiting (docs/HARNESS_LOOP.md §2). When the reset leaves secure input on in that browser (a blank tab's WebContent keeps the focused field's state until its window goes: cycle `20260919-1522` waited 50 minutes on Safari with every tab already blank, until the operator quit it by hand), the harness quits the browser (`quit saving no`, after System Events says it runs), right after the attempt (`browserReset.quit: true` on the row) or at the gate once per wait (`browserQuit` on the gate line, `BrowserQuit` in the diagnostics), under the same `benchOwnBrowser` rule, so never a browser in your use and never another application. The first Apple Event to a browser from a new terminal asks for Automation consent once: run one browser attempt attended before an unattended night.
...
Every form posts to the page that holds it (a message page takes both its File-under form and its reply form, told apart by a hidden `form` field; the shop's rows and the basket's Checkout both post to `shop/basket`), so the fixture log records the post under a key the token registered. The store answers every post with a redirect to the token's `thanks` page, so a flow registers its next step under that key: the hotel results after a search, the review page (with **Confirm reservation**) after the booking form, a received page after every check-in post ("if you have not yet, choose a seat, then finish your check-in; once Done has been posted on the finish page, you are checked in"), since the page after the finish is the same one and has to read as the finish. Page labels are checked against the policy's own click decision, not only its consequential pattern: `tests/bench-market.test.ts` runs every button on a task's route through `evaluate` as a click on the fixture host in Safari, in the default "task" mode, with the task's instruction as the user's words, and requires it to run unasked (the policy asks `Click “…”?` for any label its allow-list does not know, and in cycle 20260919-0739 that stopped every form at its own button: Apply, Review, Add to basket, Keep draft, a seat, with 7 of 11 attempts ending in a hand-off; the check-in's passenger form now says Next and its finish page Done, since the policy keeps asking at Continue and at Complete check-in, labels its patterns cannot call routine, and the task's words name neither). The only controls that ask are the ones the tasks are about (**Checkout**, which the policy reads as "Place this order?"; **Confirm reservation** and the portal's **Sign in** and password field, which the harness never approves; the CRM's **Submit**, the one prompt `ops-crm-data-entry` lists, which the instruction's own "submit it" runs without asking now that the harness passes the words). The store's one new capability is a registered body that is a PDF (`isPdf`): it is served as `application/pdf` with `Content-Disposition: attachment` under the key's last segment, so the browser downloads `<token>-invoice.pdf`; a copy left in `~/Downloads` is a token-named regular file the stray sweep removes. The `home` key means the token's root page to the store, so the smart-home panel lives at `/panel`.
...
**The files tool.** Every attempt's Runner carries the built-in files tool and nothing else of the tool layer (`src/gym/bench/tools.ts createBenchTools`, wired in both `bench.mjs` and `harness-cycle.mjs`: no Apple consent, so the bridge is never started or asked for access; no server row; the files tool over this Mac's home folder under the same path rules as in the product, docs/TOOLS.md). A note task may therefore be done the way the product does it: read the page, call `files__append_text_file` with the path the instruction names, done. Under the default "task" regime that call runs unasked because the path grounds it in the task's own words; under "all" everything does. The journal records a `tool_call` step with the frontmost application and the first-party tool id, never an argument or a result; the note graders read the file as before, `browserThenNote` accepts a tool write after the browser steps where TextEdit steps used to be required (`WRONG_ORDER` otherwise), and the soft `saved` check counts the write as the save.

**Approvals.** Every task that edits a TextEdit document lists `Save these changes?` (scoped to TextEdit as before; `files-rename-receipts` lists nothing, TextEdit is only for reading there), `ops-crm-data-entry` lists `Submit or authorize this change?` (a browser on the fixture host only), every other task nothing. Replace is never approved. The harness starts every run with the instruction as the user's own words (`taskSource: "user_words"` beside `origin: "bench"` in `attempt.ts`, as a typed or spoken command starts), so the default "task" autonomy reads it the way it reads a real request: an undoable step the words asked for ("save", "submit it", "keep it as a draft") runs and is reported instead of asking; until cycle 20260919-0739 the bench passed no source, so every such step asked and was declined.

**What a real cycle must validate.** Nothing here was run on the desktop. The first market night settles: whether Safari downloads the PDF attachment to `~/Downloads` (or asks) and how the Finder move then goes; whether the model writes dates and clock times in one of the accepted forms; how the model reads the redirect-to-`thanks` convention after a form post (a results page under `/thanks`); whether `coarena-agenda add` creates the recurring reminder's first occurrence as its `due` and prints `recurring` for it; the 15-to-30-minute window of `routine-remind-me-in` against real run lengths; whether the policy's questions land where the design expects (Place this order? on Checkout, Submit or authorize this change? on Confirm reservation, the credential refusal on the portal); whether a model told to stop before checkout ends in `request_user` with the total or says done (both pass) and whether one declined at Confirm reservation says done afterwards (`NOT_HANDED_BACK` either way); whether the model reads the check-in's received page as the finish or posts the finish page's Done again (both pass); whether Reminders sets the named start day ("starting Monday") as the first due date; and whether TextEdit saves the pre-written sort plan in place.
...
Ending reasons: `COMPLETED`, `ACTION_BUDGET`, `RUNTIME_BUDGET`, `COST_BUDGET`, `STUCK_LOOP` (the runner's loop breaker failed an unattended run that looped again after its reflection step), `POLICY_VIOLATIONS`, `STOPPED_AFTER_MANUAL_TAKEOVER`, `STOPPED_AFTER_HANDOFF`, `STOPPED_WHILE_PAUSED`, `USER_CANCELLED`, `EMERGENCY_STOP`, `HELPER_UNAVAILABLE`, `PROVIDER_FAILED`, `PROVIDER_TIMEOUT`, `RUN_ERROR`, `LEFT_IN_TAKEOVER`, `LEFT_PAUSED`, `LEFT_AWAITING_APPROVAL`, `NOT_SETTLED`.
...
| `ACTION_LOOP` / `APP_SWITCH_THRASH`                                      | `ActionLoopDetected`                                              | The same short cycle repeated (`period` 1 or 2), the same step from the same screen came round a third time within twelve (`revisits`), or the run bounced between applications.                                     |
| `LOOP_REFLECTED` / `LOOP_STUCK`                                          | `ActionLoopBroken`                                                | A loop went on past its warning with nobody to give a hint (a bench or routine run, or autonomy `all`): the run got one reflection step instead of a pause, or, looping again after it, failed as `STUCK_LOOP`.      |
...
| `DONE_CHALLENGED`                                                        | `ActionFailed DONE_CHALLENGED`                                    | The model said `done` after a step was declined or refused; the runner sent the claim back once with a fresh screenshot. With the ending: `COMPLETED` is the claim repeated, `MODEL_FAILED` the claim withdrawn.     |
...
| `APPROVAL_DECLINED_<CODE>`                                               | `UserDenied` with `approvalCode`                                  | The same decline, by the question asked as a code (`src/core/approval-codes.ts`: `SAVE_CHANGES`, `PLACE_ORDER`, `CLICK_CONTROL`, ...), which the runner stamps on the event; the question's text never is.           |
...
| `HELPER_UNAVAILABLE`, `NATIVE_ERROR`, `NATIVE_STOPPED`, `EMERGENCY_STOP` | native events                                                     | The native side. `HELPER_UNAVAILABLE` is the helper dead: exited, not answering its liveness probe, or its queue silent across two requests given up; killed, restarted, the run paused.                             |
| `HELPER_SLOW`                                                            | `NativeSlow`, `NativeError` named `HelperSlowError`               | Alive but slow: a request passed its deadline and the helper answered presence, so the wait was extended (a capture up to a minute) instead of a restart. The `NativeError` is the bound reached and the run paused. |
