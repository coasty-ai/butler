...
## Deciding with Jev (opt-in, in the app)

Despite the verdict above, Jev can be tried in the product as an **early decider** that can only make a turn faster, never change what else happens. It is off by default.
...
**How to turn it on.** Settings → Voice → Talk naturally → "Decide with Jev (TypeSafe via OpenRouter)", after entering an OpenRouter API key in the field below it. Saving refuses the toggle without a key, and a local-mode configuration saves it as off. The setting is `decisions: "off" | "jev"`. "Remove key" (or an emptied key field) turns the toggle off and deletes the stored key from the vault on the next save; a key field left alone keeps what is stored.
